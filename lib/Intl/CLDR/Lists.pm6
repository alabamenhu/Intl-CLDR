...
# It is ABSOLUTELY ESSENTIAL for the XML parse to be done in a BEGIN block.
# I highly recommend testing any modifications with a subset of languages.
# The language list here was obtained by simply ls -1 and removing .xml and line
# breaks.  %*RESOURCES does not allow access to the directories :(
...
my %collection = BEGIN {
  ...
  my $basepath = '../../../resources/main/';
  put "Generating language list.  Please be patient";
  my $completed = 0;
  my $total = +@languages;
  my $slow-down-xml = 0;
  my $slow-down-callable = 0;

  for @languages -> $language {
    # For each file in the main/ of the CLDR, find the following:
    #  - listPatterns
    #    - listPattern          # <- default, or type=and
    #      - listPatternPart type=start
    #      - listPatternPart type=middle
    #      - listPatternPart type=end
    #      - listPatternPart type=2
    #    - listPattern type=or
    ...
    my $xml = open-xml("../../../resources/main/{$language}.xml");

    # Not all XML files contain data for lists, so we can safely skip those.
    unless my @listPatterns = $xml.getElementsByTagName('listPatterns') {
      $slow-down-xml += (now - $xml-start);
      $completed++;
      next;
    }

    my @types = @listPatterns
                  .first # there should only be a single one.
                  .getElementsByTagName('listPattern');
    $slow-down-xml += (now - $xml-start);
    $completed++;


    my $callable-start = now;
    for @types -> $type {
      ...
      for @formats -> $format {
        ...
        %collection{$language-tag}{$style-type}{$format-type} =
          $logical-order
            ?? { $start ~ $^a ~ $middle ~ $^b ~ $end}
            !! { $start ~ $^a ~ $middle ~ $^b ~ $end};
      }
    }
    $slow-down-callable += (now - $callable-start);
    unless $++ mod 25 {
        put "$completed of $total: ", $slow-down-xml.floor, "s in XML, ", $slow-down-callable.floor, "s in generation";
    }
  }
  put "Generation complete."
  %collection;
}
...
sub format-list(@items is copy, :@languages is copy = user-languages, :$type = "and") is export {
  warn "Unknown list formatting type, using 'and'." unless $type eq <and or unit unit-narrow unit-short>.any;
  # this should eventually check for reduced language forms
  @languages.push("en"); # temporary default
  my $language = '';
  my $index = 0;
  $language = @languages[$index++] until %collection{$language}:exists;
  my %formatter := %collection{$language}{$type};
  given @items {
    when  0 { return ''           }
    when  1 { return @items.first }
    when  2 { return %formatter<2>(@items.first, @items.last) }
    default {
      my $last = @items.pop;
      @items.push(   %formatter<end>(   @items.pop,   $last       ));
      @items.unshift(%formatter<start>( @items.shift, @items.shift));
      @items.unshift(%formatter<middle>(@items.shift, @items.shift)) while @items > 1;
    }
  }
  return @items.first;
}
