...
When data files are written, they are currently in the extremely inefficient format of
   key subkey subsubsubkey value
which repeats for each value.  This will be made more efficient in the following two steps:
   1. replace key/subkey names with abbreviations (reduce file size, load times a little bit)
   2. use a nested serialized format (reduce file size a bit, load times a lot)
...
enum Tags <
    ...
    dates
        ...
        timeZoneNames
            ...
            zone metazone short long exemplarCity generic standard daylight
    numbers
...
for @languages -> $l {
  ...
  my $file;
  try {
    # If the file can't be opened, it probably means that there's a quote-combiningDiacritic or less-than-combiningDiacritic
    # combination that the default XML parser can't handle.
    $file = open-xml("main/{$l}.xml");
    CATCH { say "\e[1FNote: XML file for $l could not be opened                       \n"; next; }
  }

  # Ensure the file has sufficient elements
  next if $file.elements.elems == 1;
  print "\e[1FProcessing language $lang ($current-count of $total-count)\n  [√] XML opened   [ ] XML processed\r";

  # Data is designed to be cascading.  This ensures that we begin with the correct base data
  # (the sort order means we always will load 'en' before 'en-US'), or a fresh Hash.
  try {
    given $lang.comb('-').elems {
      when  0 { %results{$lang} := ($lang eq 'root' ?? Hash.new !! hash-clone %results{'root'}) }
      default {
  ...
  # Begin working with the localeDisplayNames
  ...
  with elem $file, dates {
    ...
    with elem $_, timeZoneNames {
      with elem $_, hourFormat     { safe(%m, dates, timeZoneNames, hourFormat)     = contents $_ }
      with elem $_, gmtFormat      { safe(%m, dates, timeZoneNames, gmtFormat)      = contents $_ }
      with elem $_, gmtZeroFormat  { safe(%m, dates, timeZoneNames, gmtZeroFormat)  = contents $_ }
      with elem $_, fallbackFormat { safe(%m, dates, timeZoneNames, fallbackFormat) = contents $_ }
      for elems $_, regionFormat  {
        safe(%m, dates, timeZoneNames, regionFormat, ($_<type> // 'generic')) = contents $_;
      }
      # Zones and Metazones are handled identically -- they just have slightly different types
      for elems $_, zone {
        my $type = $_<type> // "standard"; # This shouldn't ever happen, but just in case.
        with elem $_, exemplarCity { safe(%m, dates, timeZoneNames, zone, $type, exemplarCity) = contents $_ }
        with elem $_, short {
            with elem $_, generic  { safe(%m, dates, timeZoneNames, zone, $type, short, generic ) = contents $_ };
            with elem $_, standard { safe(%m, dates, timeZoneNames, zone, $type, short, standard) = contents $_ };
            with elem $_, daylight { safe(%m, dates, timeZoneNames, zone, $type, short, daylight) = contents $_ };
        }
        with elem $_, long {
            with elem $_, generic  { safe(%m, dates, timeZoneNames, zone, $type, long, generic ) = contents $_ };
            with elem $_, standard { safe(%m, dates, timeZoneNames, zone, $type, long, standard) = contents $_ };
            with elem $_, daylight { safe(%m, dates, timeZoneNames, zone, $type, long, daylight) = contents $_ };
        }
      }
      for elems $_, metazone {
          my $type = $_<type> // "standard"; # This shouldn't ever happen, but just in case.
          with elem $_, exemplarCity { safe(%m, dates, timeZoneNames, metazone, $type, exemplarCity) = contents $_ }
          with elem $_, short {
              with elem $_, generic  { safe(%m, dates, timeZoneNames, metazone, $type, short, generic ) = contents $_ };
              with elem $_, standard { safe(%m, dates, timeZoneNames, metazone, $type, short, standard) = contents $_ };
              with elem $_, daylight { safe(%m, dates, timeZoneNames, metazone, $type, short, daylight) = contents $_ };
          }
          with elem $_, long {
              with elem $_, generic  { safe(%m, dates, timeZoneNames, metazone, $type, long, generic ) = contents $_ };
              with elem $_, standard { safe(%m, dates, timeZoneNames, metazone, $type, long, standard) = contents $_ };
              with elem $_, daylight { safe(%m, dates, timeZoneNames, metazone, $type, long, daylight) = contents $_ };
          }
      }
    }
  }
  ...
  my $out = open "languages2/$lang.data", :w;
  print-hash-main $out, %m;
  $out.close;
  if my $alias-file-content = hash-alias-text(%m) {
    $out = open "languages2/aliases/$lang.data", :w;
...
#        $file.say: @keys.join($sep) ~ $sep ~ $key ~ $sep ~ $value; # BigEndian
        $file.say: $value ~ $sep ~ $key ~ $sep ~ @keys.reverse.join($sep); # Small Endian (newer more efficient method)
      }
    }
  }
}
  ...
  for %h.kv -> $key, $value {
    if $value ~~ Associative {
      ...
      if $value ~~ Alias {
        ...
        $text ~= @keys.join($sep) ~ $sep ~ $key # from
                ~ $sep2 # separate two linked sides
                ~ $value.components.map(*.Str).reverse.join($sep) ~ "\n"; # To, Small Endian
      }
    }
  }
  return $text;
...
sub get-data-abbr(--> Hash) {
    my @lines = $?PROGRAM.IO.parent.add('tag-abbreviations.data').slurp.lines.grep(none *.starts-with: '#').grep(*.chars > 0);
