...
unit class CLDR-PercentFormatSystem is CLDR-ItemNew is Positional;
...
method new(|c --> CLDR-PercentFormatSystem) {
    self.bless!bind-init: |c;
}

submethod !bind-init(\blob, uint64 $offset is rw --> CLDR-PercentFormatSystem) {
...
class Selector is Positional {
    has Int $!count;
    has Int $!length;
    has CLDR-PercentFormatSystem $!parent;
    ...
    method AT-POS ($pos) {
        ...
        my $set = $!parent!CLDR-PercentFormatSystem::sets[
                  $!parent!CLDR-PercentFormatSystem::length-table[$!length] * $!parent!CLDR-PercentFormatSystem::length-coefficient
                + $!parent!CLDR-PercentFormatSystem::count-table[$!count]
        ];
